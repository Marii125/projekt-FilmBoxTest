...
const propojeni = () => {
  ...
  let nalezenyFilm = {}; //tady potrebujem prazdny pole protoze ve foreach je parametr film ktery je jen uvnitr toho foreach a tady v tomhle pripade je to projistotu kdybchom chteli praocavt mimo forEach
  //const premieraEl = document.querySelector('#premiera'); v2
  //let jedenFilm = ''; v2
  filmy.forEach((film) => {
    if (idFilmu === film.id) {
      nalezenyFilm = film;
      let plakat = document.querySelector('#detail-filmu .img-fluid');
      plakat.src = `${film.plakat.url}`;
      let nazevFilm = document.querySelector('.card-title');
...
const starElm = (numberOfStar) => {
  star.forEach((oneStar) => {
    ...
    } else if (numberOfStar < oneStar.textContent) {
      oneStar.classList.remove('fas');
      oneStar.classList.add('far');
    }
  });
};
